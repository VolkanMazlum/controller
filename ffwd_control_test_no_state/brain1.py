...
cerebellum_application = exp.cerebellum_application  # Trial at which the cerebellum si connected to StateEstimator
...
# Motor commands relay neurons: their frequency must be scaled into 0-100 Hz
...
# Scale the feedback signal to 0-60 Hz in order to be suitable for a cerebellum comparison
# TODO Test to add a baseline rate in order to match DCN conditions (DCN have baseline around 50 Hz)
...
syn_exc = {"weight": 0.001, "delay": res} # 0.0005
...
for j in range(njt):
    if j == cereb_controlled_joint:
        ...
        # Feedback variability should be modified according to the cerebellar variability
...
# Sensory Prediction Error = Feedback - Prediction
...
'''
# Sensory Prediction Error = State - Prediction
# I cannot use the stEst population since its firing rates range from 0 to 1000 Hz
# As for sensory neurons (sn_p) I need a "feedback" population as diff neurons that
# modulate the firing rate toward 0-60 Hz
# Construct the error signal for both positive and negative neurons
nest.Connect(cerebellum.N_DCNp, error_p, {'rule': 'all_to_all'}, syn_spec=syn_inh)
nest.Connect(cerebellum.N_DCNn, error_p, {'rule': 'all_to_all'}, syn_spec=syn_exc)
nest.Connect(stEst.pops_p[cereb_controlled_joint].pop, error_p, 'all_to_all', syn_spec=syn_exc)
nest.Connect(stEst.pops_n[cereb_controlled_joint].pop, error_p, 'all_to_all', syn_spec=syn_inh)

nest.Connect(cerebellum.N_DCNp, error_n, {'rule': 'all_to_all'}, syn_spec=syn_inh)
nest.Connect(cerebellum.N_DCNn, error_n, {'rule': 'all_to_all'}, syn_spec=syn_exc)
nest.Connect(stEst.pops_p[cereb_controlled_joint].pop, error_n, 'all_to_all', syn_spec=syn_exc)
nest.Connect(stEst.pops_n[cereb_controlled_joint].pop, error_n, 'all_to_all', syn_spec=syn_inh)
'''

# Connect error neurons toward IO neurons
# w = 3.0 con fbk-fbk_cereb = 0.001 (linea 203)
...
conns_pos = nest.GetConnections(source = prediction_p, target = stEst.pops_p[cereb_controlled_joint].pop)
conns_neg = nest.GetConnections(source = prediction_n, target = stEst.pops_n[cereb_controlled_joint].pop)
...
for trial in range(n_trial):
    ...
    if trial == cerebellum_application:
        nest.SetStatus(conns_pos, {"weight": 1.0})
        nest.SetStatus(conns_neg, {"weight": 1.0})
