...
def plotPopulation_sd(time_v, N, pos_ts, pos_sds, neg_ts, neg_sds, reference, time_vecs, legend, styles, title='',buffer_size=15):
    y_p = pos_sds
    y_n = neg_sds
    if not reference:
        fig, ax = plt.subplots(2,1,sharex=True)
        ax[0].scatter(pos_ts, y_p, marker='.', s=1,c="r")
        ax[0].scatter(neg_ts, y_n, marker='.', s=1)
        ax[0].set_ylabel("raster")
        plot_rate_sd(time_v, N, pos_ts, pos_sds, buffer_size, ax=ax[1],color="r")
        plot_rate_sd(time_v, N, neg_ts, neg_sds, buffer_size, ax=ax[1], title='PSTH (Hz)')
        ax[0].set_title(title)
        ...
        for i, signal in enumerate(reference):
            ax[0].plot(time_vecs[i], signal, styles[i],label=legend[i])
            ax[0].legend()
        ax[1].scatter(pos_ts, y_p, marker='.', s=1,c="r")
        ax[1].scatter(neg_ts, y_n, marker='.', s=1)
        ax[1].set_ylabel("raster")
        plot_rate_sd(time_v, N, pos_ts, pos_sds, buffer_size, ax=ax[2],color="r")
        plot_rate_sd(time_v, N, neg_ts, neg_sds, buffer_size, ax=ax[2], title='PSTH (Hz)')
        #ax[1].set_title(title)
