...
class StateEstimator_mass:
    ...
    def __init__(self, numNeurons, numJoints, time_vect, **kwargs):
        ...
        for i in range(numJoints):
            ...
            self.pops_p = nest.Create("state_neuron_nestml", numNeurons)
            nest.SetStatus(self.pops_p, self._param_neurons)
            nest.SetStatus(self.pops_p, {"pos": True})
   
            self.pops_n = nest.Create("state_neuron_nestml", numNeurons)
            nest.SetStatus(self.pops_n, self._param_neurons)
            nest.SetStatus(self.pops_n, {"pos": False})
